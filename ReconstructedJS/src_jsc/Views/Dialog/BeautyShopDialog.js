...
// Embedded source: /Users/zhanglinsen/workspace/bianfeng/sgscq1-china-client/projects/sgscq/Resources/src_jsc/Views/Dialog/BeautyShopDialog.js:1
...
// source line 123, bytecode pc 454
(xs.Views.Dialog.BeautyShopDialogCell = cc.TableViewCell.extend({
    ccbCfg: [
        {
            name: "m_nameLab",
            tag: 2,
    ...
    (this.m_lingshi = xs.Factorys.Sprite.create("hongyan_hongyanzhixin", "hongyan"));
    // source line 70, bytecode pc 678
    this.m_lingshi.setPosition(cc.p(151, 19));
    // source line 71, bytecode pc 702
    this.m_lingshi.setScale(0.4);
    // source line 72, bytecode pc 743
    xs.Utils.Node.attachNodes(this.m_ccbNode, this.m_lingshi);
    // source line 76, bytecode pc 772
    this.m_nameLab.setAnchorPoint(xs.ap_lc);
    // source line 77, bytecode pc 801
    this.m_priceStr.setAnchorPoint(xs.ap_lc);
    // source line 81, bytecode pc 803
    return true;
},
    ...
    this._super();
    // source line 108, bytecode pc 28
    (parent = this.getParent());
    while (parent) {
        if ((parent instanceof cc.TableView)) {
            // source line 111, bytecode pc 63
            (this.m_parentView = parent);
            // source line 112, bytecode pc 81
            (touchPriority = parent.getTouchPriority());
            // source line 113, bytecode pc 98
            this.setBaseTouchPriority(touchPriority);
            break;
        }
        // source line 116, bytecode pc 121
    ...
    if (this.head) {
        // source line 125, bytecode pc 28
        this.head.removeFromParent();
    }
    // source line 129, bytecode pc 38
    (this.par = pren);
    // source line 130, bytecode pc 48
    (this.m_itemData = params);
    // source line 132, bytecode pc 67
    (num = (params.item_num || 0));
    // source line 133, bytecode pc 130
    (price = (((params.price <= params.discount_price) && (params.discount_price != 0)) ? params.discount_price : params.price));
    // source line 134, bytecode pc 151
    (residue = (params.exchange_num - params.exchanged_num));
    // source line 135, bytecode pc 163
    (mNameStrId = params.item_name);
    // source line 136, bytecode pc 168
    (mNameStr = null);
    if (xs.Utils.isSet(mNameStrId)) {
        // source line 138, bytecode pc 232
        (mNameStr = xs.Tools.String.createString(mNameStrId));
    }
    // source line 141, bytecode pc 247
    (this.m_id = params.item_id);
    // source line 143, bytecode pc 269
    (this.convertType = (params.currency_type || 0));
    // source line 144, bytecode pc 295
    (this.m_pk_id = (params.pk_id || ""));
    if ((residue <= 0)) {
        // source line 151, bytecode pc 325
        this.m_convertBtn.setEnabled(false);
    } else {
        // source line 153, bytecode pc 350
        this.m_convertBtn.setEnabled(true);
    }
    // source line 160, bytecode pc 390
    (this.iconMode = xs.Models.Item.createWithBase(this.m_id));
    // source line 162, bytecode pc 426
    (head = this.iconMode.createIcon_Grade({ num: num }));
    // source line 164, bytecode pc 460
    (btn = xs.Views.Btn.createInvisibleWithChild(head));
    // source line 165, bytecode pc 482
    btn.setPosition(this.iconPos);
    // source line 168, bytecode pc 516
    btn.setOnClickCallBack(function() {
    // source line 167, bytecode pc 39
    xs.Views.Mgr.showDialogByName("priviewGiftPackDialog", this.m_id);
}.bind(this));
    // source line 169, bytecode pc 533
    btn.setSwallowTouch(false);
    // source line 170, bytecode pc 550
    btn.setZoomOnTouchDown(false);
    // source line 172, bytecode pc 588
    xs.Utils.Node.attachNodes(this.m_ccbNode, btn);
    // source line 174, bytecode pc 598
    (this.head = btn);
    if ((this.iconMode.getItemType() !== xs.Models.ItemType_Gift)) {
        // source line 178, bytecode pc 656
        this.head.setEnabled(false);
    }
    // source line 181, bytecode pc 708
    this.m_nameLab.setString((mNameStr ? mNameStr : this.iconMode.getNameString()));
    // source line 182, bytecode pc 730
    this.m_priceStr.setString(price);
    // source line 184, bytecode pc 794
    this.labelResidueNum.setString(xs.Tools.Ml.createStringWithArgsArray("MysteryStoreDialog_convertNum", [ residue ]));
}
}));
// source line 195, bytecode pc 485
(xs.Views.Dialog.BeautyShopDialogCell.create = function() {
    var cell;
    // source line 196, bytecode pc 28
    (cell = new xs.Views.Dialog.BeautyShopDialogCell());
    if ((cell && cell.init())) {
        // source line 198, bytecode pc 60
        return cell;
    }
    // source line 201, bytecode pc 62
    return null;
});
// source line 408, bytecode pc 947
(xs.Views.Dialog.BeautyShopDialog = xs.Views.HungerLayer.extend({
    name: "xs.Views.Dialog.BeautyShopDialog",
    cfg: {
        btnClose: { priority: -10 },
        btnUpdate: { priority: -10 },
        ccbCfg: [
            { tag: 1, type: "ls", name: "dialogTitle", id: "LS_huodong_title", stringId: "BeautyDialog_title" },
    ...
    if (!this._super()) {
        // source line 225, bytecode pc 19
        return false;
    }
    ...
    (this.SumSpiritStoneNum = xs.Profile.GameData.Mgr.getInstance().Items.getTotalNumById(xs.Models.ItemID_Beauty));
    // source line 236, bytecode pc 157
    xs.log("---- 红颜币 this.SumSpiritStoneNum = ", this.SumSpiritStoneNum);
    ...
    (self = this.self);
    // source line 290, bytecode pc 21
    (parent = self.parent);
    // source line 291, bytecode pc 48
    xs.Views.Mgr.hideDialog();
    // source line 292, bytecode pc 63
    parent.updateSpiritStoneNum();
},
    updateInfo: function(resData, isInit) {
    // source line 298, bytecode pc 25
    (this.iconDataList = (resData.item_list || []));
    // source line 299, bytecode pc 95
    (this.SumSpiritStoneNum = xs.Profile.GameData.Mgr.getInstance().Items.getTotalNumById(xs.Models.ItemID_Beauty));
    // source line 300, bytecode pc 112
    this.updateDialog(isInit);
},
    refreshIconList: function() {
    // source line 307, bytecode pc 17
    ...
    this.labelCurrencySumNum.setString((this.SumSpiritStoneNum + ""));
    if (isInit) {
        // source line 317, bytecode pc 56
        this.m_tableView.reloadData();
    } else {
        // source line 319, bytecode pc 74
        this.refreshIconList();
    }
},
    onEnter: function() {
    // source line 325, bytecode pc 12
    this._super();
},
    onExit: function() {
    // source line 329, bytecode pc 12
    this._super();
},
    setBaseTouchPriority: function(priority) {
    // source line 333, bytecode pc 9
    (this.m_baseTouchPriority = priority);
    // source line 334, bytecode pc 51
    this.btnClose.setTouchPriority((this.m_baseTouchPriority + this.cfg.btnClose.priority));
    // source line 335, bytecode pc 78
    ...
    if (xs.Utils.isEmpty(itemData)) {
        // source line 341, bytecode pc 37
        return void 0;
    }
    // source line 343, bytecode pc 42
    (promptStr = null);
    // source line 344, bytecode pc 87
    (itemName = xs.Tools.String.createString(itemData.item_name));
    // source line 345, bytecode pc 180
    ...
    if ((itemPrice > this.SumSpiritStoneNum)) {
        // source line 348, bytecode pc 228
        xs.Views.Mgr.showToastByStringId("BuyPrompt_5");
        // source line 349, bytecode pc 230
        return void 0;
    }
    // source line 351, bytecode pc 286
    (promptStr = xs.Tools.String.createStringWithArgsArray("BeautyConfirmStr", [ itemPrice, itemName ]));
    // source line 387, bytecode pc 464
    ...
    leftCB: function() {
    // source line 360, bytecode pc 26
    xs.Views.Mgr.hideDialog();
}.bind(this),
    rightCB: function() {
    // source line 363, bytecode pc 26
    xs.Views.Mgr.hideDialog();
    // source line 385, bytecode pc 150
    xs.Tools.Net.requestBeautyExchange({
    ...
    if (resData.result) {
        ...
        if (info) {
            ...
            // source line 375, bytecode pc 56
            this.updateInfo(info);
            // source line 376, bytecode pc 126
            (this.SumSpiritStoneNum = xs.Profile.GameData.Mgr.getInstance().Items.getTotalNumById(xs.Models.ItemID_Beauty));
            if (add_list) {
                // source line 378, bytecode pc 171
    ...
    var cell;
    // source line 396, bytecode pc 17
    (cell = table.dequeueCell());
    if (!cell) {
        // source line 399, bytecode pc 61
        (cell = xs.Views.Dialog.BeautyShopDialogCell.create());
    }
    // source line 402, bytecode pc 89
    cell.update(this, this.iconDataList[idx]);
    // source line 405, bytecode pc 93
    return cell;
},
    numberOfCellsInTableView: function(table) {
    // source line 410, bytecode pc 11
    return this.iconDataList.length;
}
}));
// source line 415, bytecode pc 978
(xs.Views.Dialog.BeautyShopDialog.showWithSurrenderInfo = function(parent) {
    // source line 418, bytecode pc 34
    xs.Tools.Net.requestBeautyExchangeList(null, function(resData) {
    var info;
    if (resData.result) {
        // source line 420, bytecode pc 50
        xs.dump(" BeautyShopDialog", { data: resData });
        // source line 421, bytecode pc 57
        (info = resData);
        if (info) {
            // source line 426, bytecode pc 112
            xs.Views.Mgr.showDialogByName("BeautyShopDialog", info, parent);
        } else {
            // source line 430, bytecode pc 177
...
(xs.Views.Dialog.BeautyShopDialog.create = function(param, parent) {
    var ret;
    // source line 441, bytecode pc 28
    (ret = new xs.Views.Dialog.BeautyShopDialog());
...
// source line 451, bytecode pc 1117
xs.Views.Mgr.registerDialog("BeautyShopDialog", {
    "class": xs.Views.Dialog.BeautyShopDialog,
    styleType: xs.Constant_DlgStyleType_Large,
    area: [ "SecretShop", "hongyan" ]
});
