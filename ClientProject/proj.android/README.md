...
The activity and Cocos2d-x Android Java bridge are compiled by `build-apk.sh`.
`jni/` and `../Classes/` preserve the Cocos2d-x 2.2.6 + JSB native source recipe.
The native recipe targets `armeabi-v7a` and libc++; it still needs the matching
SpiderMonkey v22 Android module before `ndk-build` can produce a replacement
`libcocos2djs.so`.

## Device install test (2026-09-25)

`sgscq-rebuilt.apk` was installed and launched on the connected ADB device
(`79fac114`). The process remained alive with `SanguoshaDemo` resumed. Logcat
confirmed the recovered `main_game` path reached `main headers.js end`, routed
to `LoginScene`, and finished `main.js`. The visible screen is the local login
adapter; it performs no vendor authentication or network request. See
[`main-game-test-screenshot.png`](../main-game-test-screenshot.png).

The APK omits third-party Java SDK classes and assets. Staging restores the
original `.pg` texture payloads using the old native library's first-1,000-byte
XOR `0x18` transform, then exposes them under the `.png` paths referenced by
the game data. SceneMgr's view cleanup and overlay initialization are no-ops
for the local login adapter; recovered gameplay dialogs and the original
native JSB extension surface remain incomplete.
