...
type_map = {
    ...
    # CHAR_U/CHAR_S describe plain `char` according to target signedness;
    # only UCHAR is the C++ `unsigned char` spelling.
        ...
        if cursor.kind == cindex.CursorKind.CXX_BASE_SPECIFIER and not self.class_name in self.generator.classes_have_no_parents:
            ...
            if parent.displayname not in self.generator.base_classes_to_skip:
                ...
                if not self.generator.generated_classes.has_key(parent.displayname):
                    ...
                    # A child can mention its base before the translation-unit
                    # walk reaches the base declaration. Registering it here
                    # must also emit its JSB class when it is selected; otherwise
                    # the later walk sees the cached class and skips generation.
                    if self.generator.in_listed_classes(parent.class_name):
...
class Generator(object):
    ...
    def _deep_iterate(self, cursor, depth=0):
        ...
        if cursor.kind == cindex.CursorKind.CLASS_DECL:
            if cursor.is_definition() and len(cursor.get_children_array()) > 0 and self.in_listed_classes(cursor.displayname):
