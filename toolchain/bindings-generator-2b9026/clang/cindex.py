        ...
        if self._name_map is None:
            ...
            for key,value in CursorKind.__dict__.items():
                if isinstance(value,CursorKind):
                    self._name_map[value] = key
        return self._name_map.get(self, 'UNKNOWN_CURSOR_KIND_%d' % self.value)

    @staticmethod
    def from_id(id):
        # This generator's bundled cindex.py predates several CursorKind
        # values. Modern libclang can return those for ordinary header trees;
        # keep their numeric IDs so the generator can safely ignore kinds it
        # does not know how to emit.
        if id >= len(CursorKind._kinds) or CursorKind._kinds[id] is None:
            return CursorKind(id)
...
class TypeKind(object):
        ...
        if self._name_map is None:
            ...
            for key,value in TypeKind.__dict__.items():
                if isinstance(value,TypeKind):
                    self._name_map[value] = key
        return self._name_map.get(self, 'UNKNOWN_TYPE_KIND_%d' % self.value)
    ...
    def from_id(id):
        if id >= len(TypeKind._kinds) or TypeKind._kinds[id] is None:
            # Keep newer libclang type IDs opaque; unsupported types are
            # intentionally skipped by this legacy generator.
            return TypeKind(id)
